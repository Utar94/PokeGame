...
import type { Item, CreateItemPayload } from "@/types/items";
...
const payload = ref<CreateItemPayload>({ category: "Medicine", uniqueName: "" });
...
const onSubmit = handleSubmit(async () => {
  try {
    switch (payload.value.category) {
      case "Medicine":
        payload.value.medicine = {
        ...
        break;
      case "PokeBall":
        payload.value.pokeBall = {};
        break;
    }
    const createdItem: Item = await createItem(payload.value);
    hide();
    toasts.success("items.created");
    router.push({ name: "ItemEdit", params: { id: createdItem.id } });
  } catch (e: unknown) {
...
<template>
  <span>
    ...
    <TarModal :close="t(close)" :id="modalId" ref="modalRef" :title="t('items.title.new')">
      <form @submit.prevent="onSubmit">
        <ItemCategorySelect required v-model="payload.category" />
