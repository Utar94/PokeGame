<template>
  <div>
    ...
    <table v-if="trainers.length > 0" class="table table-striped">
      ...
      <tbody>
        <battling-trainer-row v-for="trainer in trainers" :key="trainer.id" :team="team" :trainer="trainer" />
      </tbody>
    </table>
...
import BattlingTrainerRow from './BattlingTrainerRow.vue'

export default {
  name: 'BattlingTrainerTeam',
  components: {
    BattlingTrainerRow
  },
  props: {
