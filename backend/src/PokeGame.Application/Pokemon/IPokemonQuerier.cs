...
namespace PokeGame.Application.Pokemon
{
  public interface IPokemonQuerier
  {
    ...
    Task<PokemonModel?> GetAsync(Guid trainerId, PokemonPosition position, CancellationToken cancellationToken = default);
    Task<IEnumerable<PokemonModel>> GetAsync(IEnumerable<Guid> ids, CancellationToken cancellationToken = default);
